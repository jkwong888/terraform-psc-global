...
resource "google_project" "service_project_producer" {
...
resource "google_project_service" "service_project_api_producer" {
  count                      = length(var.service_project_apis_to_enable)
  project                    = google_project.service_project_producer.id
